...
plot_age_likelihood(pars, age_at_length)
```
...
```{r}
fit <- fit_tmb_nll(pars, age_at_length = age_at_length)
...
```
If the von Bertalanffy growth rate is used for all sizes this would lead to
$$
t_0 = \frac{L_{min}}{r}+\frac{\log\left(1 - \frac{L_{min}}{L_\infty}\right)}{kr}
$$
```{r}
pars$vB_min_size / pars$r + log(1 - pars$vB_min_size / pars$L_inf) / (pars$k * pars$r)
```
...
cohort:
```{r}
u <- get_cohort_density(pars, l_max = 200, t_max = 15)
plot_density_2d(u)
```
...
```
```{r}
plot_density_3d(u, l_min = 20)
```

The solution including all yearly cohorts looks as follows:
```{r}
u_periodic <- get_periodic_number_density(pars, l_max = 200, t_max = 3)
plot_density_3d(u_periodic, l_min = 20)
```

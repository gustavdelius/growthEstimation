...
We discretise the spatial domain into cells of uniform width $\Delta l$, with cell centres at $l_i$. Using a finite volume method with semi-implicit time-stepping (Backward Euler but with rates evaluated at the earlier time step) for unconditional stability gives:
...
The flux $J_{i+1/2}^{n+1}$ is discretised using a first-order upwind scheme for advection and a central difference for diffusion:
$$J_{i+1/2}^{n+1} = \left( v_{i+1/2}^{n+} u_i^{n+1} + v_{i+1/2}^{n-} u_{i+1}^{n+1} \right) - D_{i+1/2}^n \frac{u_{i+1}^{n+1} - u_i^{n+1}}{\Delta l}$$
where $v^{n+} = \max(v^n, 0)$ and $v^{n-} = \min(v^n, 0)$.
...
We have suppressed the superscript time index $n$ above, but notice that the
matrix $A$ is evaluated at time $t^n$, not $t^{n+1}$. Our scheme is thus only semi-implicit.
